...
def calculate_composite_score(df, score_columns):
    ...
    df["score"] = sum(df[col] * weight for col, weight in zip(score_columns, weights, strict=False))
    return df
...
def create_cellrank_probability_df(
    adata_paths,
    cell_type_col,
    subject_names,
    cluster_ordering,
    ...
    for i, adata_path in enumerate(adata_paths):
        ...
        pseudotime_col_missing = (not any(pseudotime_col in key for key in adata.obsm.keys())) and (
            not any(pseudotime_col in key for key in adata.obs.keys())
        )
        if cluster_missing or pseudotime_col_missing:
            ...
            if "term_states_fwd_memberships" in pseudotime_col:
                df = pd.DataFrame(
                    adata.obsm[pseudotime_col], columns=cellrank_cols_dict[subject_name], index=adata.obs_names
                )
                df["subject"] = subject_name
                df["cell_id"] = adata.obs_names
                df["cell_type"] = adata.obs[cell_type_col].astype(str)
                df_long = pd.melt(
                    df, id_vars=["subject", "cell_id", "cell_type"], var_name="macrostate", value_name="probability"
                )
                ...
                df_long = (
                    df_long.groupby(["subject", "cell_id", "cell_type", "state"], observed=True)
                    .max()
                    .dropna()
                    .reset_index()
                )
                df = df_long.pivot(
                    index=["subject", "cell_id", "cell_type"], columns="state", values="probability"
                ).reset_index()
                ...
                df = pd.DataFrame(adata.obs[pseudotime_col_match].values, columns=["score"], index=adata.obs_names)
                df["subject"] = subject_name
                df["cell_id"] = adata.obs_names
                df["cell_type"] = adata.obs[cell_type_col].astype(str)
    ...
    if scipy.sparse.issparse(X):
        X = X.toarray()
    gene_df = pd.DataFrame(X, index=combined_adata.obs_names, columns=combined_adata.var_names)
